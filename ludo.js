...
function gettingData(){
    ...
    bluePlaying = urlQuries.get("bluePlaying")=== "true";
     ...
     redComputer = urlQuries.get("redComputer") === "true";
    greenComputer = urlQuries.get("greenComputer") === "true";
    yellowComputer = urlQuries.get("yellowComputer") === "true";
    blueComputer = urlQuries.get("blueComputer") === "true";
...
function switchPlayer(playersMove){
    return new Promise((resolve,reject) => {
        ...
        switch(playersMove){
            case 1:
                redPlaying&&!redWon ? (redsMoveToken.classList.add('floating'), resolve(playersMove)) : reject(); break;
            case 2:
                greenPlaying&&!greenWon ? (greensMoveToken.classList.add('floating'), resolve(playersMove)) : reject(); break;
            case 3:
                yellowPlaying&&!yellowWon ? (yellowsMoveToken.classList.add('floating'), resolve(playersMove)) : reject(); break;
            case 4:
                bluePlaying&&!blueWon ? (bluesMoveToken.classList.add('floating'), resolve(playersMove)) : reject(); break;
        }
    });
}
...
// Auto-play for AI
    ...
    if(isAI) setTimeout(() => autoPlay(token), 500);

// Auto-click token
function autoPlay(token){
   token.forEach(t => { if(t.classList.contains('floating')) t.click(); });
}

// Extra chance
function extraChance(){
    for(const t of token){
        if(diceOutcome===6 && t.classList.contains('floating')) return true;
    }
    return false;
}
...
function openToken(){
    ...
    tokens[playersMove-1].forEach(t => t.removeEventListener('click', openToken));

    cubePath.forEach(path => {
        let images=path.querySelectorAll('img');
        if(images.length>4) path.classList.add("makeGrid2");
        else if(images.length>1) path.classList.add("makeGrid");
    });

    switch(playersMove){
        case 1: path=document.querySelector(`.redPath1`); break;
        case 2: path=document.querySelector(`.greenPath1`); break;
        case 3: path=document.querySelector(`.yellowPath1`); break;
        case 4: path=document.querySelector(`.bluePath1`); break;
    }
...
function moveToken(){
    ...
    let i;
    switch(playersMove){
        case 1: i=parseInt([...this.parentNode.classList].find(c => c.startsWith("redPath")).substring(7)); break;
        case 2: i=parseInt([...this.parentNode.classList].find(c => c.startsWith("greenPath")).substring(9)); break;
        case 3: i=parseInt([...this.parentNode.classList].find(c => c.startsWith("yellowPath")).substring(10)); break;
        case 4: i=parseInt([...this.parentNode.classList].find(c => c.startsWith("bluePath")).substring(8)); break;
    }
    ...
    function movingToken(remaining){
        if(remaining<=0){
            ...
            cubePath.forEach(path => {
                let images=path.querySelectorAll('img');
                if(images.length>4) path.classList.remove("makeGrid2");
                else if(images.length>1) path.classList.remove("makeGrid");
            });
...
function isWon(){
    if(redHome.querySelectorAll('img').length==4 && !redWon){ redWon=true; playerWons++; return ".p1"; }
    if(greenHome.querySelectorAll('img').length==4 && !greenWon){ greenWon=true; playerWons++; return ".p2"; }
    if(blueHome.querySelectorAll('img').length==4 && !blueWon){ blueWon=true; playerWons++; return ".p3"; }
    if(yellowHome.querySelectorAll('img').length==4 && !yellowWon){ yellowWon=true; playerWons++; return ".p4"; }
}
...
function showWon(wonPlayerClass){
    ...
    switch(playerWons){
        case 1: if(!won1st){ showSpot.style.display="flex"; showSpot.innerHTML='<img src="Won1st.png" width="100%">'; won.play(); won1st=true; } break;
        case 2: if(!won2nd){ showSpot.style.display="flex"; showSpot.innerHTML='<img src="Won2nd.png" width="100%">'; won.play(); won2nd=true; } break;
        case 3: if(!won3rd){ showSpot.style.display="flex"; showSpot.innerHTML='<img src="Won3rd.png" width="100%">'; won.play(); won3rd=true; } break;
    }
}
